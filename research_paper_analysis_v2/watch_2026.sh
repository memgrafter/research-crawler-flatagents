...
# Ultralight overnight watcher for 2026 scoring + queue top-up.
#
# Usage:
#   ./watch_2026.sh
#   ./watch_2026.sh --interval 300 --target-pending 1200 --max-insert 1200
#   ./watch_2026.sh --once
#
# Env overrides also work:
#   INTERVAL_SEC=300 TARGET_PENDING=1200 MAX_INSERT_PER_CYCLE=1200 ./watch_2026.sh
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<'HELP'
Usage: watch_2026.sh [options]

Options:
  --interval SECONDS       Poll interval (default: 300)
  --target-pending N       Keep at least N 2026 pending rows (default: 1200)
...
dump_status() {
  local status_line
  status_line="$(sqlite3 ../arxiv_crawler/data/arxiv.sqlite "SELECT COUNT(*) || '|' || SUM(CASE WHEN pr.fmr_score IS NOT NULL THEN 1 ELSE 0 END) || '|' || SUM(CASE WHEN pr.fmr_score IS NULL THEN 1 ELSE 0 END) FROM papers p LEFT JOIN paper_relevance pr ON pr.paper_id=p.id WHERE p.arxiv_id LIKE '26%';")"
  local total done remaining
  IFS='|' read -r total done remaining <<< "$status_line"

  local qline
  qline="$(sqlite3 data/v2_executions.sqlite "SELECT SUM(CASE WHEN status='pending' THEN 1 ELSE 0 END) || '|' || SUM(CASE WHEN status IN ('prepping','prepped','wrapping','analyzing','analyzed') THEN 1 ELSE 0 END) || '|' || COUNT(*) FROM executions WHERE arxiv_id LIKE '26%';")"
  local pending in_flight exec_total
  IFS='|' read -r pending in_flight exec_total <<< "$qline"

  local scorer_up="no"
  if pgrep -f "relevance_scoring.main" >/dev/null 2>&1; then
    scorer_up="yes"
  fi
...
WC_DIR = Path('queries/word_clouds_2026')
...
rows = aconn.execute(
    ...
    SELECT
      ...
      pr.fmr_score AS score
    FROM papers p
    JOIN paper_relevance pr ON pr.paper_id = p.id
    WHERE p.arxiv_id LIKE '26%'
      AND pr.fmr_score IS NOT NULL
      AND pr.fmr_score >= 0.40
    ORDER BY pr.fmr_score DESC, p.arxiv_id ASC
    """
).fetchall()
aconn.close()
...
existing = {r['arxiv_id'] for r in vconn.execute("SELECT arxiv_id FROM executions WHERE arxiv_id LIKE '26%'")}
