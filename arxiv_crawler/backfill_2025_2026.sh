...
START_BOUND="2025-01-01"
END_BOUND="${END_BOUND:-$(date +%Y-%m-%d)}"
START_DATE="$START_BOUND"

# Auto-resume: read the latest successful 2025/2026 submittedDate slice from crawl_runs.
if [ -z "${BACKFILL_START_DATE:-}" ] && [ -f "$DB_PATH" ]; then
  LAST_SLICE_DAY=$(sqlite3 "$DB_PATH" "
    SELECT substr(query, instr(query, 'submittedDate:[') + 15, 8)
    FROM crawl_runs
    WHERE status = 'ok'
      AND (query LIKE '%submittedDate:[2025%' OR query LIKE '%submittedDate:[2026%')
    ORDER BY id DESC
    LIMIT 1;
  ")
...
PY
  ...
  START_DATE="${BACKFILL_START_DATE:-$START_DATE}"
fi
...
then
  ...
  echo "2025-2026 backfill already complete (next start $START_DATE is after $END_BOUND)."
  exit 0
fi
